...
def get_service_details(
    service_id: str, time_range: str = "15m", _: None = Depends(rate_limit)
):
    try:
...
def get_recent_traces(_: None = Depends(rate_limit)):
    try:
        ...
        recent_traces = []
        for bucket in (
            response.get("aggregations", {}).get("traces", {}).get("buckets", [])
        ):
            trace_id = bucket["key"]
    ...
    try:
        ...
        for hit in hits:
            ...
            span = {
                ...
                "status": (
                    "error"
                    if (src.get("status_code") and src.get("status_code", 200) >= 400)
                    or str(src.get("status")).lower() == "error"
                    else "ok"
                ),
                ...
                "end_time": src.get(
                    "timestamp"
                ),  # We would calculate this if we had proper start/end pairs instead of single logs
        ...
        return {
            "trace_id": trace_id,
            "spans": roots,
            "flat_spans": list(spans_map.values()),
        }
    except Exception as e:
...
def format_log(src):
    return {
        ...
        "message": src.get("message")
        or f"{src.get('method')} {src.get('endpoint')} [{src.get('status_code')}]",
...
def get_service_logs(
    service_name: str, limit: int = 100, _: None = Depends(rate_limit)
):
...
def alerts(_: None = Depends(rate_limit)):
    ...
    try:
        ...
        alerts = []
        for bucket in (
            response.get("aggregations", {}).get("services", {}).get("buckets", [])
        ):
            name = bucket["key"]
...
def get_rca(_: None = Depends(rate_limit)):
    try:
        ...
        if not hits:
            return {
                "message": "No critical anomalies detected. System is stable.",
                "root_cause": None,
            }
        ...
        if not trace_id:
            return {
                "message": "A failure occurred but lacks a trace_id.",
                "root_cause": "unknown",
            }
        ...
        if root_cause:
            ...
            if err_spans:
                evidence.append(
                    f"{len(err_spans)} spans returned HTTP 500+ in this trace"
                )
        ...
        return {
            "incident": "No critical anomalies detected. System is stable.",
            "root_cause": None,
        }
    except Exception:
        return {
